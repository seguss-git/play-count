...
1. **Game.** Pick a date and opponent and start, or open a game already on the schedule. Two ways to count:
   ...
   **Field view** swaps the player list for a formation diagram on a green field, with each player at their position. Tap a spot to sub that player out; the bench runs along the bottom. The formation comes from your imported chart: one row per table on the sheet, left to right in the order the columns were pasted. Toggle back with List view. A sub steps into the spot of the player he replaced; a player taken off with nobody in, or absent when his unit is loaded, leaves an open spot you can tap to fill. **Save to unit** writes the players on the field and their spots back into a unit (or a new one), so a game-day change sticks the next time that unit is tapped. Absent members keep their place unless someone else was saved into it.
...
4. **Roster.** A White and Blue switch sits at the top. White is the play-count team. Blue players exist only for practice attendance and never appear in games, play counts or units. Add jersey numbers in the # box when you have them. Players can be added one at a time or pasted like `12 Jack, 7 Owen, 22 Liam`. **Load team roster** re-adds anyone from your team link who was deleted.
   **Best fit by position** ranks the squad for any spot on the chart using combine testing: 40 time, 5-10-5, 3-cone, broad jump and weight, weighted differently per position. A star marks players already on the chart there, so gaps stand out. Scores come from percentile ranks computed from the team combine sheet; raw weights and times are deliberately not stored in this app.
   ...
   This tab opens with **Units**, the named groups you send out together such as Hawk, Viper, or Hammer. Build one with New unit and check off players, giving each a position such as `L-CB` or `R-MLB`. Or capture the group already on the field with "Use who is on the field". Units are optional and carry over between games. A Defense and Offense switch at the top of the card picks which units are listed, and **Field view** draws each one on the field, with an alternate's name in italics under his starter; tap any spot to edit the unit. A unit can also carry **alternates**: in the unit editor tap Alt beside a player who is not in the unit and give him the spot he backs up. An alternate is not sent out with the unit, but he goes in automatically when the starter at that spot is absent, and he is first on the substitution sheet for it. In a pasted chart, write the alternate after a slash, as in `Roberto/Leuffer`.
...
5. **Settings.** **My team** is a White or Blue switch that decides which team the game side belongs to. Games, play counts, units and the Counts tab all follow it, and each game and unit remembers the team that created it, so the two never mix. Practice attendance always covers both teams. That means either coach can use the same app and simply flip the switch. Also holds minimum plays per game and players on field. Season play totals. Backup and Restore move the data between phones.
   **Head coach's sheet.** For a sheet the app should never write to. Once, copy the players' names from the sheet (first player down) and paste them into this box; it shows how many matched. After a session, **Copy for sheet** on the Attendance tab copies a column of Y and blanks in the sheet's row order. Click the first player's cell under that date in the sheet and paste. Anyone here but not on the sheet, and any sheet row with no matching player, is listed so it can be fixed by hand.
...
**Opponent counts.** On the opponent side, whoever counts it types or pastes the jersey numbers from the other team's minimum play sheet (`2, 5 Smith, 7`, or one per line). Counting works like Lineup mode: tap who is on, Record Play each snap, with its own punt/PAT no-count button and Undo. Anyone under the minimum is flagged, and Copy counts puts a summary on the clipboard for the quarter reviews. The opponent roster belongs to that game only. **Edit roster** lists their players with a number box, a name box and a delete button each, plus **Clear team** to start their roster over. Removing a player who already has plays takes him out of those plays too, after a confirm.
...
**Coach PIN: who can edit.** Anyone who joins can watch live. Changing anything needs the coach PIN, typed once into Settings, Team sync. A phone without it shows "View only": every tab still works for looking, and every control that changes something is switched off. During a game, a watching phone (view only, or a coach who is not the one counting) has its own **Field view** button: it draws the players from the last recorded play on the field, placed by the depth chart, with each one's plays so far. The database enforces this itself, so it holds even against someone who skips the app. To set it up, in this order:
